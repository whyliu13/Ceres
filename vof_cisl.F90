...
real(kind=8),parameter :: thermal_delta= 0.01d0  ! for probtype=6   thickness of the thermal layer
real(kind=8),parameter :: NB_top=1.0d0, NB_bot=10.0d0  ! for probtype=6 , top and bot temperature profile
...
k1=hr*u(t(i)+0.5d0*hr,x(i)+0.5d0*k0,y(i)+0.5d0*l0)
l1=hr*v(t(i)+0.5d0*hr,x(i)+0.5d0*k0,y(i)+0.5d0*l0)
k2=hr*u(t(i)+0.5d0*hr,x(i)+0.5d0*k1,y(i)+0.5d0*l1)
l2=hr*v(t(i)+0.5d0*hr,x(i)+0.5d0*k1,y(i)+0.5d0*l1)
k3=hr*u(t(i)+hr,x(i)+k2,y(i)+l2)
l3=hr*v(t(i)+hr,x(i)+k2,y(i)+l2)
...
do while(ASSOCIATED(PR1%NEXT))
   ...
   N=N+1
ENDDO
!IF( abs( P(1) - PR1%VAL(1)) < eps .AND. (P(2).EQ.PR1%VAL(2))) THEN
...
real(kind=8)                   :: a1,b1
integer                        :: i,j
real(kind=8)                   :: fa,fb,s,c,fc,fs,d,fd,fa1,fb1
...
do while( (abs(fb).gt. 0.0d0) .and. (abs(fs).gt. 0.0d0) .and. (abs(b-a).gt.tol))
 ...
 !print *, "s=",s
   ...
   if ((( s  - 0.25d0*(3.0d0*a+b))*(s-b) .gt. 0.0d0 ) .or. &
       ...
       & (mflag .eq. 1 .and. abs(b-c) .lt. tol) .or. &
       & (mflag .eq. 0 .and. abs(c-d) .lt. tol) ) then
      s = 0.5d0*(a+b)
      mflag = 1
   else
      mflag = 0
...
if ((abs(fb).lt. eps).or.(abs(b-a).lt. tol)) then
   root = b
elseif(abs(fs) .lt. eps) then
   root = s
else
...
SUBROUTINE INIT_V(N,nox,noy,uu,vv)
IMPLICIT NONE

integer,intent(in)             :: N
real(kind=8),intent(in)        :: nox(0:N),noy(0:N)
real(kind=8),intent(out)        :: uu(0:N,0:N),vv(0:N,0:N)
INTEGER                  :: i,j
real(kind=8), external  :: u,v 

print *,"this routine has been modified to start at 0 instead of 1"

do i=0,N
   do j=0,N
   uu(i,j)= u(0,nox(i),noy(j))
   vv(i,j)= v(0,nox(i),noy(j))
   enddo
enddo
...
if(abs(x2-x1) .gt. 0.0d0) then         ! not use eps as the difference
  m = (y2-y1)/(x2-x1)
  !-------------------------------------
  if(b .ne. 0.0d0) then
...
INTEGER                              :: RX,RY
INTEGER                              :: i,j,k,p,q,i1,i2,j1,im,jm
real(kind=8)                              :: a,b,c
integer                              :: ii,jj
...
type(polygon)                         :: plgout_d,plgout_l
real(kind=8)                          :: dx(dmsn)
real(kind=8)                          :: multi_centroidA(nmat,dmsn)
integer                               :: dimen_flag
integer                               :: mof_verbose
