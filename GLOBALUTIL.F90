...
contains
       ...
       ! nbasis=1..2 * sdim if 3D
       ! nbasis=1..sdim+1 if 2D
      ...
      if ((nbasis.ge.1).and.(nbasis.le.SDIM)) then
        ...
        phi_N(2)=x0(SDIM)-rigid_centroid(SDIM)
        phi_N(SDIM)=-(x0(2)-rigid_centroid(2)) 
      else if ((nbasis.eq.SDIM+3).and.(SDIM.eq.3)) then
        phi_N(1)=x0(SDIM)-rigid_centroid(SDIM)
        phi_N(SDIM)=-(x0(1)-rigid_centroid(1)) 
      else
      ...
      if ((levelrz.eq.0).or.(levelrz.eq.1)) then
       ...
       if (rval.le.zero) then
        print *,"rval invalid in prepare_normal"
        stop
       endif
...
! if (x,y) in third quadrant, then -pi<theta<-pi/2
      ...
      do dir=1,SDIM
       if (bfact.eq.1) then
        ...
        delta=bfact*(cache_gauss_lobatto(bfact,1)+one)*dx(dir)/two
       else
        print *,"bfact invalid"
        stop
       endif
      ...
      do dir=1,SDIM
       if (bfact.eq.1) then
        ...
        xL=cache_gauss(bfact,bfact/2-1)
        xR=cache_gauss(bfact,bfact/2)
      ...
      subroutine curverr(err,LS,xsten,nhalf,dx,bfact)
      use probcommon_module
      ...
      INTEGER_T inode,jnode,knode,dir
      INTEGER_T posflag,negflag
      REAL_T dnrm(SDIM)
      REAL_T nrm(SDIM)
      REAL_T wtsum,RR,mag
      ...
      do j1=-1,1
      do k1=k1lo,k1hi
       if (LS(D_DECL(i1,j1,k1)).ge.zero) then
        posflag=1
       else
        negflag=1
       endif
      ...
      enddo 
      enddo 
      if ((posflag.eq.1).and.(negflag.eq.1)) then
       do dir=1,SDIM
        dnrm(dir)=zero
       enddo
       wtsum=zero
       do inode=-1,1,2
       do jnode=-1,1,2
       do knode=k1lo,k1hi,2
        do dir=1,SDIM
         nrm(dir)=zero
        enddo
        do i1=-1,1
         if (abs(i1-inode).le.1) then
          do j1=-1,1
           if (abs(j1-jnode).le.1) then
            do k1=k1lo,k1hi
             if (abs(k1-knode).le.1) then
              dir=1
              if (i1.eq.0) then
               nrm(dir)=nrm(dir)-inode*LS(D_DECL(i1,j1,k1))
              else  
               nrm(dir)=nrm(dir)+inode*LS(D_DECL(i1,j1,k1))
              endif
              dir=2
              if (j1.eq.0) then
               nrm(dir)=nrm(dir)-jnode*LS(D_DECL(i1,j1,k1))
              else  
               nrm(dir)=nrm(dir)+jnode*LS(D_DECL(i1,j1,k1))
              endif
              if (SDIM.eq.3) then
               dir=SDIM
               if (k1.eq.0) then
                nrm(dir)=nrm(dir)-knode*LS(D_DECL(i1,j1,k1))
               else  
                nrm(dir)=nrm(dir)+knode*LS(D_DECL(i1,j1,k1))
               endif
              endif
        ...
        if (levelrz.eq.0) then
         ...
         RR=abs(xsten(inode,1))
        else
         print *,"levelrz invalid curverr"
         stop
        endif

        dir=1
        nrm(dir)=nrm(dir)/abs(xsten(2*inode,dir)-xsten(0,dir))
        dir=2
        nrm(dir)=nrm(dir)/abs(xsten(2*jnode,dir)-xsten(0,dir))
        if (SDIM.eq.3) then
         dir=SDIM
         nrm(dir)=nrm(dir)/abs(xsten(2*knode,dir)-xsten(0,dir))
        endif

        call prepare_normal(nrm,RR,mag)

        if (levelrz.eq.0) then
         RR=one
        else if ((levelrz.eq.1).or.(levelrz.eq.3)) then
         RR=abs(half*(xsten(2*inode,1)+xsten(0,1)))
        else
         print *,"levelrz invalid curverr 2"
         stop
        endif
         
        dir=1
        dnrm(dir)=dnrm(dir)+inode*RR*nrm(dir)
        dir=2
        dnrm(dir)=dnrm(dir)+jnode*nrm(dir)
        if (SDIM.eq.3) then
         dir=SDIM
         dnrm(dir)=dnrm(dir)+knode*nrm(dir)
        endif
        wtsum=wtsum+one
       enddo
       enddo
       ...
       if (wtsum.le.zero) then
        print *,"wtsum invalid in curverr"
        stop
       endif
       if (levelrz.eq.0) then
        RR=one
        ...
        print *,"levelrz invalid curverr 3"
        stop
       endif
       dir=1
       dnrm(dir)=dnrm(dir)/(fourth*wtsum*RR*(xsten(2,dir)-xsten(-2,dir))) 
       ...
       if (levelrz.eq.0) then
        ...
        print *,"levelrz invalid curverr 4"
        stop
       endif
       ...
       if (SDIM.eq.3) then
        dir=SDIM
        dnrm(dir)=dnrm(dir)/(fourth*wtsum*(xsten(2,dir)-xsten(-2,dir)))
       endif
       ...
       do dir=1,SDIM
        err=err+dnrm(dir)
       enddo
      else if ((posflag.eq.0).or.(negflag.eq.0)) then
       err=zero
      else
       print *,"posflag or negflag invalid"
       stop
      endif
      ...
      if (im.eq.1) then
       ...
       print *,"im1 or im2 not supported yet"
       stop
      endif
      ...
      if (rzflag.eq.0) then
       if (SDIM.eq.3) then
        if (dir.eq.0) then
         area=(xsten(1,2)-xsten(-1,2))*(xsten(1,SDIM)-xsten(-1,SDIM))
        else if (dir.eq.1) then
         area=(xsten(1,1)-xsten(-1,1))*(xsten(1,SDIM)-xsten(-1,SDIM))
        else if ((dir.eq.2).and.(SDIM.eq.3)) then
         area=(xsten(1,1)-xsten(-1,1))*(xsten(1,2)-xsten(-1,2))
        else
        ...
        if (dir.eq.0) then
         area=(xsten(1,2)-xsten(-1,2))
        else if (dir.eq.1) then
         area=(xsten(1,1)-xsten(-1,1))
        else
       ...
       if (SDIM.eq.3) then
        ...
        if (dir.eq.0) then
         area=(xsten(1,2)-xsten(-1,2))* &
           two*Pi*abs(xsten(iside,1))
        else if (dir.eq.1) then
         area=(xsten(1,1)-xsten(-1,1))* &
           two*Pi*abs(xsten(0,1))
        else
         print *,"dir invalid gridarea 4"
         stop
        endif
       else
        print *,"dimension bust"
       ...
       if (SDIM.eq.3) then
        if (dir.eq.0) then
         area=abs(xsten(iside,1))*(xsten(1,2)-xsten(-1,2))* &
           (xsten(1,SDIM)-xsten(-1,SDIM))
        else if (dir.eq.1) then
         area=(xsten(1,1)-xsten(-1,1))*(xsten(1,SDIM)-xsten(-1,SDIM))
        else if ((dir.eq.2).and.(SDIM.eq.3)) then
         area=abs(xsten(0,1))*(xsten(1,1)-xsten(-1,1))* &
           (xsten(1,2)-xsten(-1,2))
        else
         print *,"dir invalid gridarea 5"
         stop
        endif
       else if (SDIM.eq.2) then
        if (dir.eq.0) then
      ...
      if (bfact.eq.1) then
       ...
       do inodes=1,bfact
        xnodes(inodes)=xlo+(ielem-lo_e)*bfact*dx+ &
         bfact*dx*(cache_gauss(bfact,inodes-1)+one)/two
       enddo
      else
       print *,"bfact invalid"
       stop
      ...
      if (bfact.eq.1) then
       ...
       do inodes=1,bfact+1
        xnodes(inodes)=xlo+(ielem-lo_e)*bfact*dx+ &
         bfact*dx*(cache_gauss_lobatto(bfact,inodes-1)+one)/two
       enddo
      else
       print *,"bfact invalid"
       stop
      ...
      INTEGER_T nhalf
      REAL_T x(-nhalf:nhalf,SDIM)
      REAL_T xlo(SDIM)
      REAL_T dx(SDIM)
      INTEGER_T fablo(SDIM)
      INTEGER_T i,j,k,bfact,dir,icrit
      ...
      INTEGER_T nhalf
      REAL_T x(-nhalf:nhalf)
      REAL_T xlo(SDIM)
      REAL_T dx(SDIM)
      INTEGER_T fablo(SDIM)
      REAL_T xdomlo,xofs,xabs,xnode
      INTEGER_T i,bfact,dir,side
      ...
      do side=-1,1,2
       ...
       do imac=1,(nhalf+1)/2
        ...
        if (bfact.eq.1) then
         ...
         i_n=ishift-i_e*bfact
         if ((i_n.ge.0).and.(i_n.lt.bfact)) then
          xnode=cache_gauss_lobatto(bfact,i_n)
      ...
      subroutine intersect_weight(ic,i,bfact_c,bfact_f,wt)
      use probcommon_module
      ...
      dir_index=1
      fablo(1)=0
      call gridsten1D(xc,xlo,ic,fablo,bfact_c,dx,dir_index,nhalf)
      dx(1)=half
      call gridsten1D(xf,xlo,i,fablo,bfact_f,dx,dir_index,nhalf)
      if (xf(1)-xf(-1).le.zero) then
       print *,"xf invalid"
       stop
      endif
      if (xc(1)-xc(-1).le.zero) then
       print *,"xc invalid"
       stop
      endif
      intlo=max(xf(-1),xc(-1))
      ...
      if (intlo.ge.inthi) then
       ...
       wt=(inthi-intlo)/(xc(1)-xc(-1))
      endif
      if ((wt.gt.-VOFTOL*dx(1)).and. &
          (wt.lt.VOFTOL*dx(1))) then
       wt=zero
      else if ((wt.lt.one+VOFTOL*dx(1)).and. &
               (wt.gt.one-VOFTOL*dx(1))) then
       wt=one
      else
       if ((wt.lt.zero).or.(wt.gt.one)) then
        print *,"wt invalid"
        stop
       endif
      endif
      ...
      return
      end subroutine intersect_weight
      ...
      subroutine intersect_weight_interpMAC(ic,i,bfact_c,bfact_f,wt)
      use probcommon_module
      ...
      if (bfact_c.eq.1) then
       if ((i/2)*2.eq.i) then
        ...
        if ((2*ic.eq.i-1).or.(2*ic.eq.i+1)) then
         wt=half
        else
         wt=zero
        endif
       ...
       if (denom*(i/denom).eq.i) then
        ...
        dir_index=1
        fablo(1)=0
        call gridsten1DMAC(xc,xlo,ic,fablo,bfact_c,dx,dir_index,nhalf)
        dx(1)=half
        call gridsten1DMAC(xf,xlo,i,fablo,bfact_f,dx,dir_index,nhalf)
        if (xf(1)-xf(-1).le.zero) then
         print *,"xf invalid"
         stop
        endif
      ...
      return
      end subroutine intersect_weight_interpMAC
      ...
      INTEGER_T nhalf
      REAL_T x(-nhalf:nhalf,SDIM)
      INTEGER_T i,j,k,level,isten,dir
      ...
      INTEGER_T nhalf,normdir
      REAL_T x(-nhalf:nhalf,SDIM)
      INTEGER_T i,j,k,level,isten,dir,ii,jj,kk
      ...
      return
         ...
         ! 1<=dir<=sdim
      subroutine intersect_weightMAC(ic,i,bfact_c,bfact_f,wt)
      use probcommon_module
      ...
      return
      end subroutine intersect_weightMAC
      ...
      subroutine tensorcomp(ux,uy,uz,vx,vy,vz,wx,wy,wz)
      IMPLICIT NONE
      ...
      subroutine strip_status(i,j,k,bfact,dir,stripstat)
      IMPLICIT NONE

      INTEGER_T i,j,k,bfact,dir,stripstat
      ...
      stripstat=0
      if (dir.eq.0) then
       if ((i/bfact)*bfact.eq.i) then
        stripstat=1
       endif
      else if (dir.eq.1) then
       if ((j/bfact)*bfact.eq.j) then
        stripstat=1
       endif
      else if ((dir.eq.SDIM).and.(SDIM.eq.3)) then
       if ((k/bfact)*bfact.eq.k) then
        stripstat=1
       endif
      else
       print *,"dir invalid"
...
!    w_{iquad}GQws(jsten,iquad,k)f_{jsten}
      ...
      INTEGER_T bfact
      INTEGER_T levelrz,dir,nc
      REAL_T RRside(2)
      REAL_T RRcen(0:bfact-1)
      ...
      if ((operation_flag.lt.0).or.(operation_flag.gt.8)) then
      ...
      if (levelrz.eq.0) then
       ...
       if (operation_flag.eq.6) then ! tensor derivatives
        ...
        if ((dir.eq.1).and.(nc.eq.2)) then

         do side=1,2
          if (RRside(side).le.zero) then
           print *,"RRside invalid"
           stop
          endif
          source_side(side)=source_side(side)/RRside(side)
         enddo ! side

         do isten=0,bfact-1
          if (RRcen(isten).le.zero) then
           print *,"RRcen invalid1 isten,RRcen=",isten,RRcen(isten)
           stop
          endif
          source(isten)=source(isten)/RRcen(isten)
         enddo ! isten

         do side=1,2
          if (bctype(side).eq.1) then  ! dirichlet bc
           if (RRside(side).le.zero) then
            print *,"RRside invalid"
            stop
           endif
           bcvalue(side)=bcvalue(side)/RRside(side)
          endif ! bctype=1?
         enddo ! side
        endif ! dir=1 and nc=2
...
!      call polyinterp_Dmatrix(bfact+1,yLRT,wLRT)
      ...
      if ((bctype(1).eq.0).and.(bctype(2).eq.0)) then ! both INT_DIR
       ...
       call poly_change_basis(bfact+1,bfact+1,PLINE,PLINE2, &
        y_extend,yGL_extend)
      else if ((bctype(1).eq.0).and.(bctype(2).eq.-1)) then !I=left E=right
       PLINE(0)=source_side(1)
       call poly_change_basis(bfact,bfact+1,PLINE,PLINE2, &
        yRTextrap,yGL_extend)
      else if ((bctype(1).eq.-1).and.(bctype(2).eq.0)) then
       do i1=0,bfact-1
        PLINE(i1)=source(i1)
       enddo
       PLINE(bfact)=source_side(2)
       call poly_change_basis(bfact,bfact+1,PLINE,PLINE2, &
        yLTextrap,yGL_extend)
      else if ((bctype(1).eq.-1).and.(bctype(2).eq.-1)) then
       do i1=0,bfact-1
        PLINE(i1)=source(i1)
       enddo
       call poly_change_basis(bfact-1,bfact+1,PLINE,PLINE2, &
      ...
      do isten=0,bfact
       ...
       if (levelrz.eq.0) then
        ...
        if (operation_flag.eq.6) then
         ...
         if ((dir.eq.1).and.(nc.eq.2)) then ! r d(v/r)/dr
        ...
        else if ((operation_flag.ge.0).and.(operation_flag.le.5)) then
         ! do nothing
      ...
      subroutine stencilbox( &
       i,j,k,fablo,fabhi,stenlo,stenhi,ng,nsten)
      IMPLICIT NONE
      ...
      subroutine aggressive_worker( &
       ...
       bfact, &
       xlo,dx, &
       scomp,ncomp,ndefined,ngrow,dir,id, &
       verbose, &
      ...
      INTEGER_T ngrow,dir,id,verbose
      ...
      if ((verbose.eq.0).or.(verbose.eq.1)) then
       ! do nothing
       ...
       if (scomp+ncomp.lt.ndefined) then
        print *,"scomp invalid"
        stop
       endif
       ...
       do dir2=1,SDIM
        print *,"dir2,lo,hi,dx,xlo ",dir2,fablo(dir2),fabhi(dir2), &
         dx(dir2),xlo(dir2)
       enddo
      ...
      return
         ...
         ! order=0 piecewise constant interp
         ! order=1 piecewise linear interp
      ...
      REAL_T x(SDIM)
      INTEGER_T lo(SDIM),hi(SDIM)
      INTEGER_T comp,ngrow,order
      INTEGER_T DIMDEC(data)
      REAL_T data(DIMV(data),comp)
      REAL_T dest
      REAL_T totalwt,localwt,int_xlo,int_xhi
      REAL_T delta(SDIM)
      REAL_T xsten_contain(-1:1,SDIM)
      REAL_T xsten_cover(-3:3,SDIM)
      INTEGER_T dir,ic,jc,kc
      INTEGER_T i1,j1,k1,nhalf
      INTEGER_T cell_index(SDIM)
      ...
      nhalf=1
      call gridsten(xsten_contain,xlo,ic,jc,kc,lo,bfact,dx,nhalf)
      do dir=1,SDIM
       delta(dir)=x(dir)-xsten_contain(0,dir)
      enddo
      do dir=1,SDIM
      do i1=-1,1
       xsten_contain(i1,dir)=xsten_contain(i1,dir)+delta(dir)
      enddo 
      enddo 
      ...
      if (order.eq.0) then
       ...
       nhalf=3
       call gridsten(xsten_cover,xlo,ic,jc,kc,lo,bfact,dx,nhalf)
       totalwt=zero
       dest=zero
       do i1=-1,1
        dir=1
        if ((i1+ic.ge.lo(dir)-ngrow).and. &
            (i1+ic.le.hi(dir)+ngrow)) then
         int_xlo=max(xsten_cover(2*i1-1,dir),xsten_contain(-1,dir))
         int_xhi=min(xsten_cover(2*i1+1,dir),xsten_contain(1,dir))
         if (int_xhi.gt.int_xlo) then
          localwt=int_xhi-int_xlo
          do j1=-1,1
           dir=2
           if ((j1+jc.ge.lo(dir)-ngrow).and. &
               (j1+jc.le.hi(dir)+ngrow)) then
            int_xlo=max(xsten_cover(2*j1-1,dir),xsten_contain(-1,dir))
            int_xhi=min(xsten_cover(2*j1+1,dir),xsten_contain(1,dir))
            if (int_xhi.gt.int_xlo) then
             localwt=localwt*(int_xhi-int_xlo)
             if (SDIM.eq.3) then
              do k1=-1,1
               dir=SDIM
               if ((k1+kc.ge.lo(dir)-ngrow).and. &
                   (k1+kc.le.hi(dir)+ngrow)) then
                int_xlo=max(xsten_cover(2*k1-1,dir),xsten_contain(-1,dir))
                int_xhi=min(xsten_cover(2*k1+1,dir),xsten_contain(1,dir))
                if (int_xhi.gt.int_xlo) then
                 localwt=localwt*(int_xhi-int_xlo)
                 totalwt=totalwt+localwt
                 dest=dest+localwt*data(D_DECL(ic+i1,jc+j1,kc+k1),comp)
                endif
               endif
              enddo ! k1
             else if (SDIM.eq.2) then
