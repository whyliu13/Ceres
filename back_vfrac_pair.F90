...
contains
  ...
  subroutine get_kappa(alpha,xsten,dir,side, &
  ...
  subroutine cell_diag_cal( &
    ...
    integer,intent(in)       :: ngeom_recon,sdim, nmat
     ...
     do dir = 1,sdim
      do side = 1,2
       ...
       if ((AFRAC.gt.0.0d0).and.(AFRAC.le.1.0d0)) then
        ...
        enddo
        call get_kappa(alpha,xsten,dir,side, &
    ...
    if (mat_osd .eq. mat_isd) then
       ...
       if (flag .eq. 1) then
          ...
          do j = -1, 1     
             ...
             if (mofdata(-1, j, datalen * (tmat - 1) + 1) .gt. eps) then
                ttag(-1, j) = 1
                lsdata(-1, j, :) = mat_cen_sten(-1, j, tmat, :)
             endif

             if (mofdata(0, j, datalen * (tmat - 1) + 1) .gt. eps) then
                ttag(0, j) = 1
       ...
       elseif(flag .eq. 3)then
          ...
          do i = -1, 1     
             if (mofdata(i, -1, datalen * (tmat-1) + 1) .gt. eps) then
                ttag(i, -1) = 1
                lsdata(i, -1, :) = mat_cen_sten(i, -1, tmat, :)
             endif

             if(mofdata(i, 0, datalen * (tmat - 1) + 1) .gt. eps) then
                ttag(i, 0) = 1
